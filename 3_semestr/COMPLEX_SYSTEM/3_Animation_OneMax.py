...
def run_genetic_algorithm():
    ...
    # Запуск алгоритма с сохранением фитнесов каждого поколения
    ...
    # === ПОСТРОЕНИЕ АНИМАЦИИ ===
    # Строим анимацию приспособленности индивидуумов по поколениям
    fig, ax = plt.subplots(figsize=(10, 6))  # Увеличенный размер графика
    ...
    ax.set_xlim(-0.5, POPULATION_SIZE - 0.5)  # Индексы индивидуумов
    ax.set_ylim(0, ONE_MAX_LENGTH + 5)  # Фитнес от 0 до 100 + запас

    # Точки для scatter plot
    scatter = ax.scatter([], [], color='#FF4500', s=20, alpha=0.6)

    # Текст для отображения номера поколения
    ...
    def update(frame):
        # Координаты: x - индексы индивидуумов, y - их фитнесы
    ...
    plt.tight_layout()
    ani.save('fitness_evolution.gif', writer='pillow', fps=5)  # Сохранение анимации в GIF
    plt.show()
